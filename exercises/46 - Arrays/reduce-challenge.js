...
function charChecker(regex) {
  return char => regex.test(char);
}

function reduceCounter(obj, item) {
  obj[item] = (obj[item] || 0) + 1;
  return obj;
}

function countLetterObjectFromText(text, testRegex) {
  const isValidChar = charChecker(testRegex);
  return text
    .split('')
    .filter(isValidChar)
    .sort()
...
const regex = /[a-z0-9]/;
const counter = countLetterObjectFromText(text.toLowerCase(), regex);

const sortedByOccurence = Object.entries(counter)
  .sort((a, b) => b[1] - a[1])
  .map(e => [e[0], e[1]]);

// const maxToMinCounter = Object.from(arr);
